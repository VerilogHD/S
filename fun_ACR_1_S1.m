function [distance_real_hori,distance_real_vert,mu,pf_hdl]=...
    fun_ACR_1_S1(dir_name,file_name,visual,imag_check,img_type,save_path)
...
% Usage: 
%   [dist_hori,dist_vert]=fun_ACR_1_S1()
%   [dist_hori,dist_vert]=fun_ACR_1_S1('dir_str','file_str',1or0,1or0)
% HW: (search for HW)
...
%8.pass/fail handle
if distance_real_vert>=188 && distance_real_vert<=192%v4
    pf_hdl(1,1)=1;
else
    pf_hdl(1,1)=0;
end
if distance_real_hori>=188 && distance_real_hori<=192%v4
    pf_hdl(1,2)=1;
else
    pf_hdl(1,2)=0;
end
